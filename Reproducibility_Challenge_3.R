...
λ2 <- 0.5
  ...
  weevil_data$weevil_poisson <- dpois(0:4, 1/avg)
weevil_data$weevil_LGP <- dLGP(0:4, 1/avg*(1-λ2),λ2)
...
weevil_plot <- weevil_plot + 
  ...
  scale_y_continuous(sec.axis = sec_axis(~.*total_count, name = "weevil_data")) 

#draw the LGP distribution curve

weevil_plot <- weevil_plot +
  geom_line(data=weevil_data, aes(x=weevil_data$k_number_of_eggs, y=weevil_data$weevil_LGP), linetype='dashed', colour='005300') +  
  geom_point(data=weevil_data, aes(x=weevil_data$k_number_of_eggs, y=weevil_data$weevil_LGP), colour='005300', shape=0, size = 3)
